...
- **Argument parsing is `node:util.parseArgs` plus a hand-rolled dispatcher**, no library. Strict mode is what turns an agent's mistyped flag into exit 2 rather than a silently ignored one. The dispatcher re-parses the *whole* argv against the resolved command — parsing the lenient pass's leftovers would mean no flag was ever checked.
- **`cli/src/registry.ts` is the single source of truth** for both dispatch and help, so help cannot drift. A structural test walks it and asserts every command declares a usage line and does not shadow a global flag.
- **Exit codes: 0 / 1 / 2** (ok / failed / bad command line). `--json` puts `{"ok":true,"data":…}` on stdout and `{"ok":false,"error":…}` on stderr.
